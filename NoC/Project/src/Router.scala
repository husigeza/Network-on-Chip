...
class Input_port extends Module {
  val io = new Bundle {
    ...
    val Flits_transmitted_test = UInt(OUTPUT,width=3)
    val Header_arrived_test = UInt(OUTPUT,width=1)
    val Transmission_ongoing_test = UInt(OUTPUT,width=1)
  ...
  io.Header_arrived_test := Header_arrived
  ...
  /*For Testing*/
  io.Input_Channel_Header_Flit := Input_Channel_Header_Flit
  io.Input_Channel_Data_Flit := Input_Channel_Data_Flit  
  io.Header_arrived_test := Header_arrived
  io.Flits_transmitted_test := Flits_transmitted
  io.Transmission_ongoing_test := Transmission_ongoing
  /*****************************************************************/
...
class ArbiterUnit extends Module {
  val io = new Bundle {
    ...
    //TESTING
    val Shift_reg_test = UInt(OUTPUT, width = 32)
    val End_reg_test = UInt(OUTPUT)
    val X_reg_test = UInt(OUTPUT,width=3)
    val Y_reg_test = UInt(OUTPUT,width=3)
  ...
  io.Shift_reg_test := Shift_reg
  io.End_reg_test := End_reg
  io.X_reg_test := X_reg
  io.Y_reg_test := Y_reg
...
class Router extends Module{
   val io = new Bundle {
    ...
    val Port_1_input_output_to_crossbar = UInt(OUTPUT,width=8)
    val Port_P_input_output_to_crossbar = UInt(OUTPUT,width=8)
    ...
    val Cross_bar_Port_3_Output = UInt(OUTPUT, width = 8)
    val X_reg_test = UInt(OUTPUT,width=3)
    val Y_reg_test = UInt(OUTPUT,width=3)
    
    val Port_P_Flits_transmitted_test = UInt(OUTPUT,width=3)
    val Port_P_Transmission_ongoing_test = UInt(OUTPUT,width=1)
    
    val Port_3_Output_ready_to_Crossbar = Bool(OUTPUT)
    
      val Crossbar_ready = Bool(OUTPUT)
    val Processor_Data_Valid_to_Crossbar = Bool(OUTPUT)
   } 
   ...
   /*TESTING*/
    io.Crossbar_ready := Cross_Bar.io.Crossbar_ready
     io.Processor_Data_Valid_to_Crossbar := Processor_Input_port.io.Data_Valid_to_Crossbar  
  io.Input_Channel_Header_Flit :=   Processor_Input_port.io.Input_Channel_Header_Flit
  io.Input_Channel_Data_Flit :=   Processor_Input_port.io.Input_Channel_Data_Flit
  
  io.Port_P_input_output_to_crossbar := Processor_Input_port.io.Data_Out
 
  io.Port_P_END := Processor_Input_port.io.END
  io.Port_1_END := Port_1_Input_port.io.END  
  io.Port1_enable := Cross_Bar.io.Port1_enable 
  io.PortP_enable := Cross_Bar.io.PortP_enable
  io.Processor_Header_arrived_test := Processor_Input_port.io.Header_arrived_test
  io.Shift_reg_test := Cross_Bar.io.Shift_reg_test
  io.End_reg_test := Cross_Bar.io.End_reg_test
 
  io.Cross_bar_Port_3_Output := Cross_Bar.io.Port3_output
  io.X_reg_test := Cross_Bar.io.X_reg_test
  io.Y_reg_test := Cross_Bar.io.Y_reg_test
  //io.Port_3_Output_ready_to_Crossbar := Port_3_Output_port.io.Output_Port_Ready
  io.Port_3_Output_ready_to_Crossbar := Cross_Bar.io.PortP_to_input_Output_Ready
  io.Port_P_Flits_transmitted_test := Processor_Input_port.io.Flits_transmitted_test
  io.Port_P_Transmission_ongoing_test := Processor_Input_port.io.Transmission_ongoing_test
}
